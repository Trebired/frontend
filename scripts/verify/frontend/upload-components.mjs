...
async function verifyUploadStyles(rootDir) {
  ...
  assert.ok(upload.includes("--tbf-ui-upload-surface-bg"));
  ...
  assert.ok(upload.includes("--tbf-ui-upload-surface-state-drop-border-style"));
  assert.ok(upload.includes("[data-tbf-upload-drag]"));
  assert.ok(upload.includes("--tbf-ui-upload-surface-state-drag-shadow"));
  assert.ok(upload.includes("white-space: nowrap"));
  assert.ok(upload.includes("--tbf-ui-upload-hint-line-h"));
  assert.ok(upload.includes("--tbf-ui-upload-meta-gap, var(--tbf-gap-sm, 8px)"));
  assert.ok(upload.includes('data-tbf-upload-entry-count="1"'));
  assert.ok(upload.includes("--tbf-ui-upload-list-max-h"));
  assert.equal(upload.includes("tbf-upload__button"), false);
  assert.equal(upload.includes("--tbf-ui-upload-btn"), false);
  assert.ok(cropper.includes("--tbf-ui-upload-cropper-stage-overlay-color"));
  assert.ok(cropper.includes("--tbf-ui-upload-cropper-stage-line-contrast-color"));
  assert.ok(cropper.includes("--tbf-ui-upload-cropper-stage-line-width, 1px"));
  assert.ok(cropper.includes("--tbf-ui-upload-cropper-stage-point-inset"));
  ...
  assert.ok(cropper.includes("--tbf-ui-upload-resp-mobile-cropper-stage-height"));

...
async function verifyUploadStyles(rootDir) {
  ...
  assert.ok(upload.includes('ns.css-var("ui-upload-surface-bg")'));
  assert.ok(upload.includes("grid-template-columns: auto minmax(0, 1fr)"));
  assert.ok(upload.includes('var(#{ns.css-var("border-width")}, 1px) solid var(#{ns.css-var("border")}, #000)'));
  assert.ok(upload.includes('ns.data("upload-drop", "true")'));
  assert.ok(upload.includes('ns.css-var("ui-upload-surface-state-drop-border-style")'));
  assert.ok(upload.includes('ns.data("upload-drag")'));
  assert.ok(upload.includes('ns.css-var("ui-upload-surface-state-drag-shadow")'));
  assert.ok(upload.includes("white-space: nowrap"));
  assert.ok(upload.includes('ns.css-var("ui-upload-hint-line-h")'));
  assert.ok(upload.includes('var(#{ns.css-var("ui-upload-meta-gap")}, var(#{ns.css-var("gap-sm")}, 8px))'));
  assert.ok(upload.includes('ns.data("upload-entry-count", "1")'));
  assert.ok(upload.includes('ns.css-var("ui-upload-list-max-h")'));
  assert.equal(upload.includes("tbf-upload__button"), false);
  assert.equal(upload.includes("--tbf-ui-upload-btn"), false);
  assert.ok(cropper.includes('ns.css-var("ui-upload-cropper-stage-overlay-color")'));
  assert.ok(cropper.includes('ns.css-var("ui-upload-cropper-stage-line-contrast-color")'));
  assert.ok(cropper.includes('ns.css-var("ui-upload-cropper-stage-line-width")}, 1px'));
  assert.ok(cropper.includes('ns.css-var("ui-upload-cropper-stage-point-inset")'));
  ...
  assert.ok(cropper.includes('ns.css-var("ui-upload-resp-mobile-cropper-stage-height")'));

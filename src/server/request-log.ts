...
function requestLogKey(ctx: RequestContext) {
  return `${ctx.method} ${requestLogPath(ctx)}`;
}
...
function browserProbePrefixes(options: FrontendRequestLogOptions) {
...
function isQuietBrowserProbe404(pathname: string, options: FrontendRequestLogOptions) {
...
function shouldSkipFinishedRequestLog(
  ctx: RequestContext,
  status: number,
  options: FrontendRequestLogOptions,
) {
  ...
  return new Set(options.quietSuccessRoutes || []).has(requestLogKey(ctx));
}
...
function readRequestLogger(options: FrontendRequestLogOptions) {
...
function attachFrontendRequestLogger(
  app: any,
  options: FrontendRequestLogOptions = {},
) {
...
export { attachFrontendRequestLogger };
export type { FrontendRequestLogOptions };
